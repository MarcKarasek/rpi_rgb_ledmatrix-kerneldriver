...
static int usage(const char *progname) {
  ...
  fprintf(stderr, "Options:\n"
          "\t-s <srvr addr>:<port> : Server Address:Port to use for Net Interface\n"
...
int main(int argc, char *argv[]) {
  ...
  int opt;
  while ((opt = getopt(argc, argv, "dlD:t:r:p:c:m:w:L:s:")) != -1) {
    switch (opt) {
    case 'D':
      demo = atoi(optarg);
      break;
    ...
    case 's':
        servAddress = optarg;
        pos = servAddress.find(':');
        host = servAddress.substr(0, pos);
        if (pos != std::string::npos);
        {
                port = servAddress.substr(pos+1);
        }
        length = port.copy(pnum,5,0);
        pnum[length]='\0';
        portnum = atoi(pnum);
        cout << "Parsed Server Address " << host << " Port " << portnum << endl;
        break;
  ...
  if (host.empty())
  {
      cout<<"Must Specify a Server Address and Port"<<endl;
      return usage(argv[0]);
  }
